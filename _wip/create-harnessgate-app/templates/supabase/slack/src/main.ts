...
// Connect all active Slack apps from the DB
const apps = await getActiveApps("slack");
for (const app of apps) {
  const creds = app.credentials as { botToken: string; appToken: string };
  const appId = await bridge.connect("slack", { botToken: creds.botToken, appToken: creds.appToken });
...
console.log(`HarnessGate running — ${apps.length} Slack bot(s) listening`);
